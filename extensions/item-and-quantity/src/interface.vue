...
export default {
	...
	setup(props, { emit }) {
		...
		const estoqueCollection = `estoque`
		//estoqueCollection
		//relatedCollection.value.collection
		const primaryKey = fieldsStore.getPrimaryKeyFieldForCollection(estoqueCollection);
		...
		function useRelation() {
			const relation = computed(() => {
				return relationsStore.getRelationsForField(props.collection, "name")?.[0];
			});
